...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
    	...
    	//create output map to append later
    	Map<T3Action, T3State> trans = new HashMap<>();
    	
    	//first find if player is odd or even
    	int[] moves = this.getMoves();
    	
    	//great, now we can add these to any squares... there are a LOT of valid transitions
    	//O(r*n) isn't bad so just iterate through them because I have other homework (circuits is rough)
    	//oh, yo, also could just use these without variable names but readable code !
    	int r = this.state.length;
    	int c = this.state[0].length; //DEBUGGING: might need to switch r,c. if errors, check here 
    	T3Action temp; //my C days still haunt me so i refuse to declare within loop
    	
    	//i in range [0,r] ; j in range [0,c] ; k in range [0,|moves|]
    	for(int j = 0; j < c; j++)
    	{
    		for(int i = 0; i < r; i++)
    		{
    			...
    			{
    				temp = new T3Action(j,i,moves[k]); //col, row, move (Why do column first?? ewww)
    				if(this.isValidAction(temp))
    				{
    					trans.put(temp, this.getNextState(temp));
    				}
    				else
    				{
    					//if we can't place a thing here then it's a taken spot so nothing works here
    					break;
    				}
